...
1. **Failing test** at whatever seam reaches the bug — a Pest/PHPUnit unit, feature, or integration test.
2. **HTTP script** — `curl` against a running dev server (`php artisan serve`, `herd`, sail).
3. **Artisan / CLI invocation** with a fixture input, diffing output against a known-good snapshot. A throwaway `artisan tinker` script or a one-off command counts.
4. **Headless browser script** (Playwright) — drives the UI, asserts on DOM/console/network. Use the `playwright-cli` skill.
...
- Faster? (Cache setup, skip unrelated init, narrow `--filter` to the one test.)
- Sharper signal? (Assert on the specific symptom, not "didn't throw".)
- More deterministic? (Pin time with `Carbon::setTestNow`, seed the faker, isolate the DB, freeze the network.)
...
1. **Debugger / REPL inspection** if the env supports it (`tinker`, Xdebug breakpoint). One breakpoint beats ten logs.
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), note the specifics and suggest running `/review` against the diff to catch related issues. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
